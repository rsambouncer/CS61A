...
GUI.prototype.selectAnt = function(name, cost, img) {
    this.selected_ant = { name: name, cost: cost, img: img };
}
...
$('#antsTableRow').on('click', ".ant-row", function() {
    ...
    gui.selectAnt($(this).attr('data-name'), $(this).attr('data-cost'), $(this).attr('data-img'));
});
...
$('.places-table').on('click', '.places-td', function() {
    ...
    //Deselect ant
    //gui.deselectAnt(); //whom'st added this line and why? -cheese
    if (!selectedAnt) {
        swal({
            title: "Error",
    ...
    else if (selectedAnt["cost"] > gui.get_food()) {
        swal({
            title: "Error",
        ...
        $.ajax({
            ...
            .done(function(response) {
                if (response["error"]) {
                    ...
                    if(response["id"] == -1){
                      //do nothing, the ant will soon be removed
                    }
                    else if(!gui.locToAnt[r][c]){
                      gui.locToAnt[r][c] = [response["id"]];
                    }else{
                      // container, or empty list from removal
    ...
    for (bee in newLocation) {
        if (oldLocation[bee] != newLocation[bee]) {
            ...
            if (img.css("position") != "absolute") {
                ...
                img.css({"margin-top": "40px", "top": currentLocTop, "left": currentLocLeft, "position": "absolute", "pointer-events":"none"});
            }
